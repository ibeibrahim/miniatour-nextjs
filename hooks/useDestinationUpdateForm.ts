import { DestinationType, UpdateDestinationData } from "@/types/types";
import { useState } from "react";

interface UseDestinationUpdateFormProps {
  initialData: DestinationType | null;
  onSubmit: (data: UpdateDestinationData) => Promise<DestinationType | null>;
  onSuccess?: () => void;
}

export function useDestinationUpdateForm({
  initialData,
  onSubmit,
  onSuccess,
}: UseDestinationUpdateFormProps) {
  const [data, setData] = useState({
    ...
    operational_hours: initialData?.operational_hours || "",
  });
