...
interface DestinationCreateFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (destination: DestinationType) => void;
}

export function DestinationCreateFormModal({
  isOpen,
  onClose,
  onSuccess,
}: DestinationCreateFormModalProps) {
  const { createDestination, loading: loadingDestination } = useDestinations();
  const { getCities, loading: loadingCity } = useCities();
  const [cities, setCities] = useState<CityType[] | []>([]);
  const loading = loadingDestination || loadingCity;

  const form = useForm();
  const handleSubmit = async (data: CreateDestinationData) => {
            ...
            <div className="grid gap-6 py-4">
              ...
              <div className="grid gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem disabled value="0">Choose the City</SelectItem>
                    {cities.map((city, key) => (
              ...
              {/* Name Field */}
              <div className="grid gap-2">
                <FormLabel htmlFor="destination_name">Destination Name</FormLabel>
                <Input
                  id="destination_name"
                ...
                {errors.destination_name && (
                  <p className="text-sm text-red-500">{errors.destination_name}</p>
                )}
              </div>
              ...
              <div className="grid gap-2">
                <FormLabel htmlFor="operational_hours">Operatinal Hour</FormLabel>
                <Input
                  id="operational_hours"
                  value={data.operational_hours ?? ""}
                  onChange={(e) => updateField("operational_hours", e.target.value)}
                  placeholder="Enter destination operational hours"
                  className={errors.name ? "border-red-500" : ""}
                  disabled={loading}
                />
                {errors.operational_hours && (
                  <p className="text-sm text-red-500">{errors.operational_hours}</p>
                )}
              </div>
            </div>
