...
import { useAppointmentUpdateForm } from "@/hooks/useAppointmentUpdateForm";

interface AppointmentUpdateFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function UpdateAppointmentFormModal({
  isOpen,
  onClose,
  appointment,
  onSuccess,
}: AppointmentUpdateFormModalProps) {
  const { updateAppointment, loading: loadingAppointment } = useAppointment();
  ...
  const handleSubmit = async (data: UpdateAppointmentData) => {
